    ...
    <div class="py-6 relative">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" v-if="!isLoading">
        ...
        <div v-else>
          ...
          <!-- 选中分类的VPS管理 -->
          <div v-if="selectedCategoryCollection">
        <!-- 选择VPS分类 -->
        <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
          <div class="px-4 py-5 sm:px-6">
            <h3 class="text-lg leading-6 font-medium text-gray-900">选择VPS分类</h3>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">请选择您想要管理的VPS分类</p>
          </div>
          <div class="border-t border-gray-200 px-4 py-5 sm:p-6">
            <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-3 sm:gap-x-6">
              <div 
                    @click="selectedCategory = 'vpsDeals'" 
                class="cursor-pointer p-4 border rounded-lg transition-all duration-300 category-card"
                    :class="selectedCategory === 'vpsDeals' ? 'border-blue-500 bg-blue-50 transform scale-105' : 'border-gray-300 hover:border-blue-300'"
              >
                <div class="flex items-center space-x-3">
                  <div class="flex-shrink-0">
                    <div class="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center category-icon">
                  ...
                  <div>
                        <div class="text-lg font-medium text-gray-900">VPS</div>
                        <div class="text-sm text-gray-500">虚拟专用服务器</div>
                  </div>
                </div>
              </div>
              
              <div 
                    @click="selectedCategory = 'vdsDeals'" 
                class="cursor-pointer p-4 border rounded-lg transition-all duration-300 category-card"
                    :class="selectedCategory === 'vdsDeals' ? 'border-green-500 bg-green-50 transform scale-105' : 'border-gray-300 hover:border-green-300'"
              >
                <div class="flex items-center space-x-3">
                  <div class="flex-shrink-0">
                    <div class="h-10 w-10 rounded-full bg-green-500 flex items-center justify-center category-icon">
                      <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div>
                        <div class="text-lg font-medium text-gray-900">VDS</div>
                        <div class="text-sm text-gray-500">虚拟专用服务器，接近独立服务器性能</div>
                  </div>
                </div>
              </div>
              
              <div 
                    @click="selectedCategory = 'dedicatedDeals'" 
                class="cursor-pointer p-4 border rounded-lg transition-all duration-300 category-card"
                    :class="selectedCategory === 'dedicatedDeals' ? 'border-indigo-500 bg-indigo-50 transform scale-105' : 'border-gray-300 hover:border-indigo-300'"
              >
                <div class="flex items-center space-x-3">
                  <div class="flex-shrink-0">
                    <div class="h-10 w-10 rounded-full bg-indigo-500 flex items-center justify-center category-icon">
                      <svg class="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <div>
                        <div class="text-lg font-medium text-gray-900">独立服务器</div>
                        <div class="text-sm text-gray-500">物理独立服务器，适合高性能需求</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-6">
              <div class="px-4 py-5 border-b border-gray-200 sm:px-6 flex justify-between items-center">
                <div>
                  <h3 class="text-lg leading-6 font-medium text-gray-900">{{ selectedCategoryName || categoryTitle }} 列表</h3>
                  <p class="mt-1 max-w-2xl text-sm text-gray-500">
                    <span v-if="categoryDeals.length > 0">当前共有 {{ categoryDeals.length }} 个{{ selectedCategory === 'dedicatedDeals' ? '独立服务器' : (selectedCategory === 'vdsDeals' ? 'VDS' : 'VPS') }}</span>
                    <span v-else>暂无数据，请点击右侧添加按钮添加新的{{ selectedCategory === 'dedicatedDeals' ? '独立服务器' : (selectedCategory === 'vdsDeals' ? 'VDS' : 'VPS') }}</span>
                  </p>
                </div>
          ...
          >
              ...
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    ...
                    添加{{ selectedCategory === 'dedicatedDeals' ? '独立服务器' : (selectedCategory === 'vdsDeals' ? 'VDS' : 'VPS') }}
          </button>
          <button 
            @click="confirmReset" 
                    class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors"
          >
                    <svg class="h-4 w-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
            </svg>
              ...
              <!-- VPS卡片列表 -->
              <div v-if="categoryDeals.length > 0" class="px-4 py-6">
...
export default {
  ...
  setup() {
    ...
    // 状态变量
    const selectedCategory = ref('vpsDeals');
    ...
    // 新增选中的分类集合
    const selectedCategoryCollection = ref(null);
    ...
    const handleCategorySelect = (categoryData) => {
      if (categoryData && categoryData.collection) {
        selectedCategoryCollection.value = categoryData.collection;
    ...
    const categoryDeals = computed(() => {
      return store.getters.getDeals(selectedCategory.value);
    });
    
    // 当前分类标题
    const categoryTitle = computed(() => {
      if (selectedCategory.value === 'vpsDeals') return 'VPS 列表';
      if (selectedCategory.value === 'vdsDeals') return 'VDS 列表';
      if (selectedCategory.value === 'dedicatedDeals') return '独立服务器 列表';
      return 'VPS 列表';
    });
